...
class PPOTrainer(BaseTrainer):
    ...
    def compute_rewards(
        ...
        这段代码实现了PPO算法中的计算奖励函数的过程，具体来说，它包括以下公式：
        reward[last_non_masked_index] += score - self.kl_ctl.value * (logprob - ref_logprob)
        其中，reward表示第t个动作的奖励，last_non_masked_index表示最后一个非零元素的索引，score表示偏好模型的得分，logprob表示当前动作的对数概率，ref_logprob表示参考动作的对数概率，
        self.kl_ctl.value表示KL惩罚项的系数。这个公式的含义是，将偏好模型得分和KL惩罚项相加，
        得到score - self.kl_ctl.value * (logprob - ref_logprob)，然后将其加到第t个动作的奖励中
        """
    ...
    def loss(
        ...
        def compute_loss(self, data, last_values):
            ...
            # 优势函数（Advantage）的计算：优势函数用于衡量当前状态或行动相对于期望值的优劣程度。A(s,a) = Q(s,a) - V(s)
            for t in reversed(range(gen_len)):
                # 获取下一个时间步的值，如果当前时间步是最后一个时间步，则使用 last_values
                nextvalues = values[:,t + 1] if t < gen_len - 1 else last_values
                # 计算 delta，即当前时间步的奖励加上折扣因子乘以下一个时间步的值减去当前时间步的值
                delta = rewards[:, t] + self.config.gamma * nextvalues - values[:,t]
                # 计算 GAE-Lambda，即 delta 加上折扣因子乘以 Lambda 倍的上一个时间步的 GAE-Lambda
                lastgaelam = delta + self.config.gamma * self.config.lam * lastgaelam
                # 将计算得到的 GAE-Lambda 添加到 advantages_reversed 列表中
                advantages_reversed.append(lastgaelam)
            # 将 advantages_reversed 列表转换为张量，并进行维度转置
            ...
            # 计算统计信息
            entropy = masked_mean(entropy_from_logits(logits), masks)
            approxkl = 0.5 * masked_mean((old_logprobs - logprobs) ** 2, masks)
            policykl = masked_mean(old_logprobs - logprobs, masks)
            return_mean, return_var = masked_mean(returns, masks), masked_var(returns, masks)
            value_mean, value_var = masked_mean(values, masks), masked_var(values, masks)
            stats = dict(
                ...
                policy=dict(
                    ...
                    advantages=advantages.detach(),
                    advantages_mean=masked_mean(advantages, masks).detach(),
                    ratio=ratio.detach(),
                ),
                returns=dict(mean=return_mean.detach(), var=return_var.detach()),
                val=dict(
                    vpred=masked_mean(values, masks).detach(),
                    error=masked_mean((values - returns) ** 2, masks).detach(),
    ...
    def record_step_stats(self, kl_coef: float, **data):
        """
        这段代码对应于 PPO 算法中记录训练步骤统计信息的函数。具体来说，这个函数计算了 KL 散度、熵、平均非分数奖励、平均分数和标准差，并记录了这些统计信息。
        在函数中，首先从 data 中弹出 masks 数据，然后计算 KL 散度和熵。接下来，计算平均非分数奖励、平均分数和标准差，并在 KL 散度变得负数时发出警告
        然后，函数记录了一些统计信息，包括 KL 散度、KL 散度分布、策略函数的对数概率、参考策略函数的对数概率、KL 系数、熵、平均非分数奖励、平均分数和标准差等。此外，还记录了文本属性，如查询长度、响应长度等。
        最后，函数记录了 PPO 算法的训练统计信息，并返回  把这些补充道注释中
        """
        mask = data.pop("masks")
        ...
        # 记录记录了一些统计信息，包括 KL 散度、KL 散度分布、策略函数的对数概率、参考策略函数的对数概率、KL 系数、熵、平均非分数奖励、平均分数和标准差等
        stats = {
